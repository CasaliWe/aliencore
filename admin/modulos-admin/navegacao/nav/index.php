<?php
    ...
    if(strpos($urlAtual, 'dashboard') !== false){
        ...
        $activeProjetos = true;
    }else if(strpos($urlAtual, 'servicos') !== false){
        $tituloContentPagina = "Serviços";
        $activeServicos = true;
    }else{
        $tituloContentPagina = "Informações";
        $activeDashboard = true;
...
<nav class="d-flex flex-column w-100 mt-5 pt-5 mt-lg-0 pt-lg-0">
    ...
    <a href="servicos.php" class="link-nav-desktop <?= $activeServicos ? 'active-link-desktop' : ''; ?>">Serviços</a>
